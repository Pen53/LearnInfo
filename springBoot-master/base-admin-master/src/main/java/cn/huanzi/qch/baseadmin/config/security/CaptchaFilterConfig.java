...
import cn.huanzi.qch.baseadmin.util.RsaUtilRedis;
...
public class CaptchaFilterConfig implements Filter {
    ...
    @Autowired
    private RsaUtilRedis rsaUtilRedis;
        ...
        if ("POST".equals(request.getMethod()) && "/login".equals(request.getRequestURI().replaceFirst(contextPath,""))) {
            ...
            if("Y".equals(SysSettingUtil.getSysSetting().getSysApiEncrypt())){
                ...
                try {
                    ...
                    //后端私钥解密的到AES的key
                    byte[] plaintext = rsaUtilRedis.decryptByPrivateKey(Base64.decodeBase64(aesKey), rsaUtilRedis.getPrivateKey());
            ...
            //从session中获取生成的验证码
            String verifyCode = session.getAttribute("verifyCode")!=null?session.getAttribute("verifyCode").toString():"";
            System.out.println("6666-------------------verifyCode:"+verifyCode);
            if (captchaEnable && !verifyCode.toLowerCase().equals(request.getParameter("captcha").toLowerCase())) {
                ...
                if("Y".equals(SysSettingUtil.getSysSetting().getSysApiEncrypt())){
                    ...
                    String aesKey = Base64.encodeBase64String(rsaUtilRedis.encryptByPublicKey(key.getBytes(), publicKey));
